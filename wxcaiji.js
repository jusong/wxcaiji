    ...
    *beforeSendResponse(requestDetail, responseDetail) {
        ...
        else if(/\/s\?__biz/i.test(requestDetail.url) || /mp\/rumor/i.test(requestDetail.url) || /appmsg\/show\?__biz/i.test(requestDetail.url))  //公众号文章接口（rumor这个地址是公众号文章被辟谣了）
        {
            ...
            try {
                ...
                var original_url = $('#js_share_source').attr('href');

                var reg = /var _copyright_stat = \"(.*?)\";/;//历史消息
                var ret = reg.exec(bodyStr);
                var copyright_stat = ret[1];
                ...
                return new Promise((resolve, reject) => {
                    ...
                    HttpPost({content:content, original_url:original_url, copyright_stat:copyright_stat, url:requestDetail.url},"/wxCaiji/saveMsgContent", function (body) {
                        if (body) {
